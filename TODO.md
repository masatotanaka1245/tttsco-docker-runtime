...
| P1 | 未着手 | 回答直前の適合性ガードを拡張し、`相談 / 提案 / 資料メモ追記 / 集計 / 報告書化` の意図ずれも検知して、短文でも誤回答をそのまま出荷しないようにする | `AI_System_Data/src/AnswerAlignmentChecker.php`, `AI_System_Data/src/ChatEvaluationPolicy.php`, `AI_System_Data/src/ChatEvaluator.php`, `AI_System_Data/src/LightweightFinalAnswerGuard.php`, `AI_System_Data/src/AdvancedRouteFinalizer.php` | 意図ずれ時に `[FINAL-GUARD]` または evaluator が `clarify / reject / revise_text_only` を返し、誤回答が保存や FAQ 化まで進まないことを確認する |
| P1 | 未着手 | `project memory` 自動更新の品質ゲートを強化し、評価未実施や意図ずれ回答で `AGENTS / README / TODO` 自動メモが汚染されないようにする | `AI_System_Data/src/ProjectMemoryAutoUpdater.php`, `AI_System_Data/src/ChatEvaluator.php`, `AI_System_Data/src/LightweightFinalAnswerGuard.php` | `[PROJECT-MEMORY-AUTO] skipped=quality_guard` が適切に出ること、高品質回答だけが自動メモへ反映されることを確認する |
...
| P1 | 未着手 | FAQ auto-registration の条件を見直し、`lightweight_rule_guard` の高品質回答も候補に含めるかを実装で確定する | `AI_System_Data/src/FaqAutoRegistrar.php`, `AI_System_Data/public/api/chat_normal.php`, `AI_System_Data/public/api/chat_analysis.php`, `AI_System_Data/src/AdvancedRouteFinalizer.php` | `evaluation_mode=real` と `evaluation_source=lightweight_rule_guard` の扱いが設計どおりになり、`[FAQ-AUTO]` の判定理由が追えることを確認する |
