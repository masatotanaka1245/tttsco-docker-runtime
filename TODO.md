...
| --- | --- | --- | --- | --- |
| P1 | 進行中 | fallback / missing eval 時に、未完了回答や操作案内だけの回答を最終回答として通さず確認質問へ倒す条件を整理する | `AI_System_Data/src/ChatEvaluator.php`, `AI_System_Data/src/LightweightFinalAnswerGuard.php`, `AI_System_Data/src/EvaluationRecoveryCoordinator.php`, `AI_System_Data/public/api/chat_normal.php`, `AI_System_Data/public/api/chat_analysis.php`, `AI_System_Data/src/AdvancedRouteFinalizer.php` | `evaluation_mode=fallback` / `evaluation_source=judge_fallback` / `evalResult=null` のときに、最終回答本文を確認質問へ倒す条件と影響範囲を整理する |
