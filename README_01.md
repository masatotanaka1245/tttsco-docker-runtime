...
- `README_01.md` は、このアプリの現行仕様と機能を定義する設計図です。
- 画面構成、データベース構造、主要ルート、現行機能、分割済み責務を記録し、実装判断の基準にします。
- 新しい仕様や画面・DBの変更を入れた場合は、このファイルも追随更新します。
- 実装時は、このファイルにない独自仕様や架空のカラムを勝手に追加しないことを原則とします。
...
- 画面仕様
  ...
  - 中央画面はプレビュー中心
  - `新規資料` / `編集` / `削除` はタブ右上に配置
  - 編集はモーダルで行い、保存後は中央プレビューと左一覧を Ajax で即時更新する
- 主な保存 API
  - `public/api/save_material.php`
  - `public/api/get_material.php`
  - `public/api/delete_material.php`
  - `public/api/save_material_note.php`
- 主な実装ファイル
  - `public/support.php`
  - `public/templates/modals.php`
  - `public/assets/js/support.js`
  - `src/ProjectMaterialDocumentService.php`
- 運用メモとの違い
...
1. 実データ（DB、CSV、PDF本文、`doc_chunks`）
...
- 分解 packet の役割
  ...
  - 単純質問は無理に細分化せず、1件の `sub_question` のまま扱う
  - route を強制的に決めるのではなく、後段の route 判定や follow-up 継続の補助材料として使う
...
- `project_meta` の auto TODO の役割
  ...
  - `進行中` は常に1件だけ維持し、残りを `未着手` / `検証中` / `完了` / `保留` に分ける
- 導入順
  - 質問分解
  - 分解ログ保存
  - durable TODO 反映
...
この構成では、`chat_reasoning_steps` を「その場の分解ログ」、`ai_project_todo_auto_md` を「次回にも効く持続タスク」として責務分離する。
...
- 本番ログまたは返却 payload の `model_roles` を使って、各 route の責務分担が意図どおり動いているか検証する
- `diagram_mode=on` の `登録済みCSVの概要` では、本文に加えて `json:chart` が返ることを確認する
...
- `advanced_hybrid`
  - 因数分解、最終統合、最終リライト、品質評価は `main`
  - SQL生成や補助分析などの中間処理は `sub`
- `global`
  - 最終レポート生成は `main`
  - ReAct ループと中間調査は `sub`

### `data_analysis` への適用方針メモ

`data_analysis` は、すべてを `sub` に寄せるのではなく、以下のように段階を分けて扱う方針とします。
...
- `sub` に寄せるもの
  - Text-to-SQL の生成と再生成
  - CSV証拠読解のバッチ分析
  - `column_semantics` / `semantic_category_summary` / `category_filtered_distribution` の補助推定
  - 追加抽出キーワードや追加サブクエリの補助生成
- AIを使わない deterministic 処理
  - `date_histogram`
  - `distinct_count`
  - `exact_value_count`
  - `value_distribution`
  の SQL 実行と整形は、そのまま PHP / SQL 主体で維持する
...
CSV生成	回答モードの `CSV化` が有効な場合、AI回答内の Markdown 表を案件CSVとして登録し、CSVタブから再利用できる。`csv化してください` のような依頼は軽量 `data_analysis.csv_export_request` へ寄せ、重い証拠読解を避ける	public/api/chat.php, public/api/chat_analysis.php, src/CsvExportGenerator.php, src/CsvSummaryFormatter.php, public/assets/js/modules/chat.js
回答モードUI	support.phpのチャット欄に「フル思考」「図解」「報告書」「CSV化」の小型スイッチを配置。生成中ステータスは入力欄上の控えめなバーに表示し、詳細は完了後の折りたたみログで確認	public/support.php, public/assets/js/modules/chat.js, public/assets/css/styles.css
...
9. `csv_mode=on` の場合は、回答本文中の Markdown 表を `CsvExportGenerator.php` が案件CSVとして登録する。軽量CSV export route では、まず deterministic な表を優先生成し、その結果をCSVタブへ追加する。
...
- 直近ログ（2026-06-04）を踏まえた次の改善方針として、自然文の「月別」「年月」「日時は不要」「若い順」「特定月を抽出して件数」といった質問は、単一CSVの `data_analysis.csv_agg` を第一候補とし、`advanced_hybrid` や汎用 `value_distribution` へ落とさない方向で整備を進める。
- 2026-06-05 の見直しでは、内部の責務分離だけでは体感品質が上がりきらなかったため、主要ユーザーフローごとに「期待 route」「引き継ぐ文脈」「`report_mode` / `diagram_mode` の効き方」を先に固定してから route 修正へ戻る方針に切り替えた。
- 特に次の 2 点を優先課題として扱う。
...
- 2026-06-05 の見直しで、履歴要約に `report_mode=on` が付いている場合は、文面に `報告書` が明示されなくても軽量 `history_summary` より報告書化フローを優先する方針へ寄せた。
- `diagram_mode=on` は図表生成を許可するが、CSV要約・CSV集計の軽量ルートは維持する。
- `advanced_reasoning=on` でも、CSV要約やCSV集計は軽量 `data_analysis` を優先するよう調整済み。
- 軽量ルートでも最終出荷前に軽量品質確認を行い、必要なら追加検索ではなく既存根拠のみで文章修正する。
...
- `グラフにしてください。` のような短い図表要求でも、小規模CSV概要では LLM rewrite に頼らず deterministic な `json:chart` を優先する。
...
- 2026-06-12 の時点で、`chat_advanced.php` の重い責務は次の helper へ分割済み。
  - `AdvancedSqlSubQueryRunner`: SQL 生成、preset SQL、修復、実行 fallback
  - `AdvancedFastPathResolver`: 履歴報告書 fast path、multi-source advice fast path
  - `AdvancedRouteCompletionCoordinator`: lightweight final guard、保存、最終送信
  - `AdvancedReasoningStepRecorder`: reasoning step 保存、Map-Reduce 進捗保存
  - `AdvancedBulkCsvMapReducer`: CSV 全件読解の 10件分割 Map-Reduce
  - `AdvancedFinalDraftGenerator`: 軽量最終回答ルート、通常の最終ドラフト生成
  - 現在の `chat_advanced.php` は、オーケストレーション中心へ寄せている

直近着手方針（2026/06/04）

- 第1優先は、CSV の月別・年月系集計の自然文解釈を安定化すること。
- 具体的には、以下を同じ改善束として扱う。
  - 自然文の `月別` / `年月` / `日時は不要` を `date_histogram(month)` に寄せる
  - `2026年4月を抽出して件数` のような質問を、全体分布ではなく exact count に寄せる
  - 追い質問でも、直前の CSV 名と列名を補完して軽量 `CSV-AGG` に戻す
- 第2優先は、全社横断ブリーフィング系の route とログ表現の整合性を高めること。
