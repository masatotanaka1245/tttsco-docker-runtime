...
7. 報告書モードの場合は、回答保存後に `ReportGenerator.php` がHTML/CSS報告書を作成し、Composerで導入した mPDF を優先してPDF化する。mPDFが利用できない検証環境では wkhtmltopdf または Chrome/Edge headless にフォールバックする。生成PDFは `documents` と `doc_chunks` に登録され、アップロードPDFと同じようにPDFタブとRAG検索で利用できる。
