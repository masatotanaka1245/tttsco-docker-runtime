...
- `これまでの会話内容を簡潔にまとめてください` のような履歴要約は、`report_mode=on` でも `history_summary` を優先する
...
- `これまでの会話内容を簡潔にまとめてください` は、`report_mode=on` でも `history_summary` に入り、重い `advanced_hybrid` へ吸われないことを確認する
