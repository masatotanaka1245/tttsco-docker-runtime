...
class PromptManager {
    ...
    public static function getProjectOperatingMemoryInstruction(array $memoryDocs): string
    {
        ...
        return "\n【案件運用メモ】\n"
             ...
             . "4. 自動生成メモは、現在スレッドと案件全体の最近会話から作られた補助要約であり、次回の回答方針に優先的に反映してよい情報です。ただし、実データや資料本文と矛盾する場合はそちらを優先してください。\n"
             . "5. 自動生成メモの中に `改善ログ` セクションがある場合、そこに書かれた `改善策` と `次回ルール` は、最近の回答品質評価から得られた運用上の学習結果として扱ってください。特に `次回ルール` は、次の回答時に先回りして適用してよい行動ルールです。\n"
             . "6. 手動メモは補助的な補正・上書き候補として扱ってください。自動生成メモと手動メモが矛盾する場合は、まず自動生成メモを主仮説として採用し、必要に応じて手動メモの補足を参照してください。\n"
             . "7. 資料本文・DB実データ・FAQ・コメントと矛盾する場合は、実データと資料本文を優先してください。\n\n"
             . implode("\n\n", $sections) . "\n";
    }
}
