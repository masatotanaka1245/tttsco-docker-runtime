...
class PromptManager {
    ...
    public static function getCommonInstructions(): string {
        return "\n【RAG（資料参照）に関する絶対遵守事項】\n"
             ...
             . "6. 【図表の出力】 ユーザーから「図解して」「グラフ・フロー図で示して」と要求された場合は、必ずMermaid.jsの記法（```mermaid ... ```）を用いて描画コードを出力してください。\n"
    ...
    public static function getDatabaseMemoryInstruction(?string $jsonStr): string {
        ...
        try {
            ...
            $instruction = "\n【現在のデータベース構造と実在するバリューの記憶】\n"
                         ...
                         . "1. 【ハルシネーションの徹底窒息】あなたは上記の「記憶」に実在するテーブル名、および実在するカラム・キー名（大文字・小文字の区別を厳守、row_data->>\"\$.name\" や Name など）のみを使用して思考・クエリ生成を行わなければなりません。上記に存在しない項目をあなたの想像ででっち上げることは固く禁じます。\n"
                         ...
                         . "3. 【複数テーブルの安全横断】構造化データ（project_csv_rows）と非構造化資料（doc_chunks）の繋がりが明記されている場合、それぞれのテーブルの境界を守りつつ、自律的に安全に跨いで（JOINや多段階ステップで）クロス集計・分析を組み立ててください。\n";
