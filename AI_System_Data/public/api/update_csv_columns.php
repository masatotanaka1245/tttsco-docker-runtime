<?php
/**
 * update_csv_columns.php - 手作業CSV台帳の列追加・削除API
 */
...
if (!$projectId || !$csvFileId || !canManageProject($pdo, $projectId, $userId, $role)) {
    http_response_code(403);
    echo json_encode(['success' => false, 'error' => 'この案件のCSV列を編集する権限がありません。'], JSON_UNESCAPED_UNICODE);
    exit;
}

if (!is_array($headers)) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => '列データの形式が不正です。'], JSON_UNESCAPED_UNICODE);
    exit;
}

try {
    $service = new ProjectCsvTableService($pdo);
    $csvFile = $service->updateColumns($projectId, $csvFileId, $headers);
