<?php
/**
 * create_csv_table.php - 手作業CSV台帳の新規作成API
 */
...
if (!$projectId || !canManageProject($pdo, $projectId, $userId, $role)) {
    http_response_code(403);
    echo json_encode(['success' => false, 'error' => 'この案件へCSV台帳を作成する権限がありません。'], JSON_UNESCAPED_UNICODE);
    exit;
}

if (!is_array($headers)) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => '列名の形式が不正です。'], JSON_UNESCAPED_UNICODE);
    exit;
}

try {
    $service = new ProjectCsvTableService($pdo);
    $csvFile = $service->createManualCsv($projectId, $fileName, $headers);
