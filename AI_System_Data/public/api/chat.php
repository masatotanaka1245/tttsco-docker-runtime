...
try {
    ...
    $routeFactorizer = new ChatRouteFactorizer($csvContextResolver, 'chatLogger', $projectMemoryDocs);
    ...
    if (($factorizedQuery['route'] ?? null) === 'advanced_hybrid.history_report' && !$report_mode) {
        $report_mode = true;
        chatLogger("[SMART-ROUTER] 会話履歴の報告書化要求に合わせて report_mode を自動有効化しました。");
        chatLogger("[OUTPUT-MODE] report_mode=on | diagram_mode=" . ($diagram_mode ? 'on' : 'off') . " | source=route_auto");
    }
