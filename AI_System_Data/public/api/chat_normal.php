...
class NormalStreamingRouteProcessor {
    ...
    private function buildSystemPrompt(): string {
        ...
        if ($this->shouldSkipDocumentRagForCodeImprovement()) {
            $system_prompt .= "\n【コード改善の実装計画モード】この質問は、短い実装指示としてまとめてください。"
                            . "\n- 回答は 1800〜2400 文字程度を目安にすること"
                            . "\n- 見出しは「方針 / 最小変更 / 確認 / 残課題」の最大4つに固定すること"
                            . "\n- 各見出しの箇条書きは2〜4個までにすること"
                            . "\n- 長い背景説明、コード全文、最終報告テンプレは出さないこと"
                            . "\n- 変更対象 / 最小差分 / 確認方法 / 残課題だけに絞ること"
                            . "\n- 全体を網羅するより、次に Codex が実装しやすい最小指示を優先すること";
        }
        if ($this->isProjectMemoryConsultationRoute()) {
    ...
    private function buildCodeImprovementProjectMemoryPrompt(string $projectOperatingMemoryPrompt): string
    {
        ...
        $lines = [
            ...
            '- 回答は、変更対象 / 最小差分 / 確認方法 / 残課題を先に整理してください。',
            '- 見出しは「方針 / 最小変更 / 確認 / 残課題」の最大4つに固定してください。',
            '- 各見出しの箇条書きは2〜4個までに抑え、長い背景説明は省いてください。',
            '- コード全文や最終報告テンプレは出さず、次に実装しやすい最小指示を優先してください。',
            '- CSV / PDF / 報告書の成果物そのものではなく、回答品質や実装挙動を改善する依頼として扱ってください。',
        ];
