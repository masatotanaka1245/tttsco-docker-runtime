...
function isAutoGeneratedThreadTitle(title) {
    ...
    if (/^会話\s+\d+$/.test(normalized)) return true;
    return normalized === '既存会話' || normalized === '新しい会話';
}

function buildThreadTitleFromMessage(message) {
    let normalized = String(message || '').replace(/\s+/g, ' ').trim();
...
function syncActiveThreadTitleFromMessage(message, threadId) {
    if (!threadId) return;

    const nextTitle = buildThreadTitleFromMessage(message);
    if (!nextTitle) return;

    const activeTitleEl = document.getElementById('active-thread-title');
    if (activeTitleEl && isAutoGeneratedThreadTitle(activeTitleEl.textContent || '')) {
        activeTitleEl.textContent = nextTitle;
    }

    const threadButton = document.querySelector(`[data-thread-switch][data-thread-id="${String(threadId)}"]`);
    const threadTitleEl = threadButton ? threadButton.querySelector('[data-thread-title]') : null;
    if (threadTitleEl && isAutoGeneratedThreadTitle(threadTitleEl.textContent || '')) {
        threadTitleEl.textContent = nextTitle;
    }
}
...
function handleChat(e) {
    ...
    appendMsg('user', msg);
    syncActiveThreadTitleFromMessage(msg, threadId);
    input.value = '';
