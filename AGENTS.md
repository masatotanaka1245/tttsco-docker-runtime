...
- `project_meta` の案件運用メモと `project_comments` の案件コメントは別物として扱う
...
- 作業中にエラー、詰まり、環境依存問題、誤判定が発生した場合は、まず原因を切り分ける
- 同じ問題が再発しそうな場合は、`TROUBLESHOOTING.md` に短く記録する
- `TODO.md` は「次にやること」だけを書く。エラー履歴や作業ログを溜め込まない
- `TROUBLESHOOTING.md` には、症状・原因・対処・再発防止・関連ファイルを書く
- password、token、cookie、個人情報、DB dump の中身は記録しない
- Git / Docker / DB / network / permission / encoding の問題は、再発しやすいため優先して記録する
- push 失敗時は、DNS失敗 / 認証失敗 / 環境ポリシー拒否 / remote rejection を区別して報告する

### 3.3.1 案件運用メモの扱い

- `AGENTS / README / TODO` 相当の案件メモは、回答生成の補助コンテキストとして使う
- 案件運用メモの保存先は `project_meta` の `ai_project_agents_md` / `ai_project_readme_md` / `ai_project_todo_md` であり、`project_comments` と混同しない
- 入口の粗い質問因数分解でも、`README / AGENTS / TODO` をまとめて補助参照してよい
- 案件メモは現在、案件状態と直近会話から自動更新される
- 自動生成メモを優先参照し、手動メモは補助・補正として扱う
- 手動編集内容は自動生成メモとは分離して保持し、必要な補足や修正を人が入れる前提で扱う
- 自動更新では、現在スレッドの最近の会話を優先しつつ、案件全体の最近の会話も薄く補助参照してよい
- ただしチャット回答時は、長い会話履歴よりも `ProjectContextMemory`、資料、CSV、直前の成果品ステートを優先する
- 会話履歴は原則として現在スレッドの直近2〜3件を主に使い、長い履歴全文を主コンテキスト化しない
- `TODO` の自動生成では、`進行中` を 1 件だけ選び、`未着手` / `検証中` / `完了` / `保留` を分けて扱う
- `TODO` の `進行中` は、直近メッセージの丸写しではなく、未完了の依頼を優先して選ぶ。返答済みの依頼は `検証中` または `完了` へ回す
- 返答済みでも評価スコアが低い依頼は `完了` にせず、`検証中` に残して再確認対象とする
- 回答時は、`TODO` の `進行中` を現在の主成果品レーンとして扱い、ユーザーが明示的に別レーンを指示するまで継続候補にしてよい
- 今後の自動 TODO では、全質問共通の質問分解 packet から `save_worthy` な未完了タスクだけを durable TODO に反映する方針とする
- `chat_reasoning_steps` は分解ログと進捗トレースの保存先であり、durable な案件TODOそのものとして扱わない
- 確認質問、一時仮説、`save_worthy=false` の step は `ai_project_todo_auto_md` にそのまま保存しない
- `AIエージェント` の自動生成メモには `改善ログ` が含まれることがあり、そこに書かれた `改善策` と `次回ルール` は次の回答時の行動ルールとして優先参照してよい
- ただし、資料本文やDB実データより優先して断定の根拠にしてはいけない
- `TODO` は特に、作業中メモや仮説を含み得るため、事実ソースとして引用しない
- `改善ログ` も、回答のふるまい改善には使ってよいが、資料本文やDB実データより優先して事実断定に使わない
...
- 案件メモを追加した場合は、少なくとも以下を確認する
  ...
  - `AIエージェント` では `改善ログ（次回ルール）` が自動欄に分離表示されること
...
- 資料は単発の回答本文ではなく、PDFや報告書へ育てていく中間成果品として扱う
- 資料作成・追記・修正・章立て相談では、PDF要約より先に既存の資料メモを確認し、差分更新前提で扱う
- `CSV / PDF / 資料メモ / project_comments / project_faqs / 案件情報` は、成果品を完成させるための情報源として横断参照してよい
...
- 今後は、route 判定前に全質問共通の軽量 decomposition packet を作る方針とし、複合質問だけを `sub_question` 単位で扱う
- 単純質問は無理に分解しすぎず、1件の質問として扱ってよい
- 因数分解の目的は以下とする
  - どのルートへ送るべきかを安定化する
  - 単一CSV / 全CSV / PDF / 案件全体の取り違えを減らす
  - 集計 / 要約 / 検索 / 比較の意図混線を減らす
  - 改善フェーズでの判定修正を、場当たりの正規表現追加だけにしない
- decomposition packet は「advanced へ送るか」「normal に残すか」の補助材料として使い、ここで SQL 実行可否まで決め打ちしない
...
- 背景
  - 直近数件だけでは継続文脈が弱く、全文履歴ではノイズが強い
  - 複合質問や follow-up では、直前の成果品レーンと未完了タスクが履歴に残っている方が安定する
- やってはいけないこと
  - 履歴全文をそのまま prompt に積むこと
  - 長い assistant 本文で summary を埋めること
  - 会話履歴を `project_meta`、資料、CSV、PDF より優先すること
- 推奨対応
  - factorizer には現在スレッドの短い recent raw history を使う
  - prompt には deterministic な compact thread summary を使う
  - 主コンテキストは引き続き `project_meta`、資料、CSV、PDF、直前の成果品ステートを優先する
  - 変更時は、compact history が冗長化していないか、`project_id=null` で保存系が落ちないか、`save_worthy=false` が durable TODO に残っていないかを確認する
...
- `これまでの会話内容を要約` のような履歴要約でも、`report_mode=on` や `報告書` / `レポート` の明示がある場合は、軽量 `history_summary` より報告書化フローを優先する
...
### 3.10 成果品ステート継続の教訓（2026-06-16）

- 背景
  - CSV 集計の follow-up で、`target_file_name` と `target_column` は補完できても、直前の `aggregation_mode` や `wants_chart`、`sort_order` が落ちると `CSV-OVERVIEW` へ脱線しやすいことが分かった。
  - `これまでの会話内容をまとめて報告書化` のような依頼でも、`history_summary` が先に走ると、成果品化ではなく単なる履歴要約へ弱く着地してしまう。
- やってはいけないこと
  - 会話履歴を、単なる単語補完元としてだけ扱うこと
  - follow-up の短い追加指示を、新規の広い質問として再解釈し、毎回 route を考え直すこと
  - `report_mode=on` や `報告書` / `レポート` の明示があるのに、軽量 `history_summary` を優先すること
- 推奨対応
  - 会話履歴から `target_file_name` / `target_column` だけでなく、`aggregation_mode` / `sort_order` / `wants_chart` / `output_format` を含む `成果品ステート` を復元する
  - `data_analysis.csv_agg` の成功後に来る `若い順で` / `グラフ化して` / `逆順で` のような追い指示は、`ChatRouteSelector` で route lock して同じ成果品レールを継続する
  - 履歴の報告書化は、`history_summary` より `advanced_hybrid` または報告書専用フローを優先し、要約ではなく成果品出荷の意図を守る

### 3.11 成果品駆動運用の教訓（2026-06-16）

- 背景
  - これからのチャット体験は、単なるQ&Aではなく、`運用メモ`、`資料`、`PDF`、`CSV`、`ナレッジ` を会話しながら育てる形へ寄せていく。
  - このとき、案件運用メモと案件コメントを混同したり、会話履歴を長く抱え込み過ぎたりすると、成果品の現在地より古い文脈に引っ張られやすい。
- やってはいけないこと
  - `project_meta` の運用メモと `project_comments` を同じものとして扱うこと
  - 成果品の現在地より、長い会話履歴全文を優先してプロンプトへ積むこと
  - FAQ / PDF / CSV の生成を、毎回必ず人手クリック前提と決めつけること
- 推奨対応
  - `ProjectContextMemory`、資料、CSV、直前の成果品ステートを、会話履歴より優先して参照する
  - 会話履歴は現在スレッドの直近2〜3件を主に使い、インテント検知と follow-up 継続へ絞る
  - FAQ / PDF / CSV は、ユーザーの意図と route 条件が揃っていれば AI が直接生成・登録してよい
  - ただし、破壊的変更や正式公開のような高リスク操作には、将来的に HITL を追加できる余地を残す

### 3.12 実ログで確認した脱線パターンの教訓（2026-06-16）

- 背景
  - `案件の内容を詳細にまとめてください。` や `次はどう進めたら良いでしょうか。` が `advanced_hybrid` に入ったあと、`multi_source_advice` ではなく PDF抽出寄りの `semantic_extract` に倒れ、資料の OCR ノイズやテンプレ断片を主回答にしてしまった。
  - `統合_.csvを集計し、グラフにしてください。` は `detail=data_analysis.csv_agg` で着地しているのに、実処理は `CSV-EVIDENCE` へ落ち、長時間かかったうえに推定値ベースの回答になった。
  - assistant 回答本文の `主な項目` のような見出しが、次の follow-up で `target_column=主な` として誤補完された。
- やってはいけないこと
  - `advanced_hybrid.multi_source_advice` の意図を下流へ渡さず、曖昧な相談や案件要約を PDF抽出に吸い込ませること
  - `グラフ` / `チャート` を含む単一CSV依頼を、列未指定のまま `CSV-EVIDENCE` へ落として長時間読解させること
  - assistant 回答の見出し語や説明語を、実在カラム確認なしで `target_column` 候補に採用すること
- 推奨対応
  - `route_detail=advanced_hybrid.multi_source_advice` を advanced 側へ引き渡し、相談・案件要約は deterministic fast path を優先する
  - `グラフ` / `チャート` を含む単一CSV依頼は、まず `CSV-AGG` に固定し、列未指定なら確認質問で止める
  - assistant 履歴から列名を拾う場合は、実在する `project_csv_files.column_headers` と一致する候補だけを採用する
